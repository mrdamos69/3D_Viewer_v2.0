...
int s21::Model::is_digit(char expression) {
  int error = 1;
  if ((expression >= '0' && expression <= '9') || expression == '.') error = 0;
  return error;
}
...
bool s21::Model::parsing_vertexes_and_polygons(std::string &path_of_file,
                                               int &cnt_vertexs,
                                               int &cnt_polygons,
                                               data_t &some_data) {
      ...
      if (lineptr[0] == 'v' && lineptr[1] != 'n' && lineptr[1] != 't') {
        for (int index = 2;
          ...
          if (is_digit(lineptr[index]) == 0 || lineptr[index] == '-') {
            ...
            while (is_digit(lineptr[index]) == 0) index++;
