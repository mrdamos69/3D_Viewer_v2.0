...
TEST(MODEL, move_obj) {
  ...
  for (int i = 0; i < some_data.matrix.get_rows(); i++)
    for (size_t j = 0; j < 3; j++) some_data.matrix(i, j) = 0;

  test.move_obj(some_data, 1, 1, 1);
  for (int i = 0; i < some_data.matrix.get_rows(); i++)
    for (size_t j = 0; j < 3; j++) ASSERT_DOUBLE_EQ(some_data.matrix(i, j), 1);
}
...
TEST(MODEL, scale_obj) {
  ...
  for (int i = 0; i < some_data.matrix.get_rows(); i++)
    for (size_t j = 0; j < 3; j++) some_data.matrix(i, j) = 1;

  test.scale_obj(some_data, 2);
  for (int i = 0; i < some_data.matrix.get_rows(); i++)
    for (size_t j = 0; j < 3; j++) ASSERT_DOUBLE_EQ(some_data.matrix(i, j), 2);
}
...
TEST(MATRIX, s21_sum_matrix_test_1) {
  ...
  for (int i = 0; i < mtx.get_rows(); i++)
    for (int j = 0; j < mtx.get_cols(); j++) mtx_result(i, j) = 33;
  try {
...
TEST(MATRIX, s21_sub_matrix_test_1) {
  ...
  mtx -= mtx_2;
  for (int i = 0; i < mtx.get_rows(); i++)
    for (int j = 0; j < mtx.get_cols(); j++) mtx_result(i, j) = 0;
  try {
    EXPECT_EQ(mtx_result.eq_matrix(mtx), true);
  } catch (const char* e) {
    std::cout << e << std::endl;
...
TEST(MATRIX, s21_mult_number_test_1) {
  ...
  mtx *= 2;
  for (int i = 0; i < mtx.get_rows(); i++)
    for (int j = 0; j < mtx.get_cols(); j++) mtx_result(i, j) *= 2;
  try {
    EXPECT_EQ(mtx_result.eq_matrix(mtx), true);
  } catch (const char* e) {
    std::cout << e << std::endl;
